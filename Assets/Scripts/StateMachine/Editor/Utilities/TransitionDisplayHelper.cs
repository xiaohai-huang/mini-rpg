...
namespace UOP1.StateMachine.Editor
{
    internal class TransitionDisplayHelper
    {
        ...
        internal TransitionDisplayHelper(
            SerializedTransition serializedTransition,
            TransitionTableEditor editor
        )
        {
            SerializedTransition = serializedTransition;
        ...
        internal bool Display(ref Rect position)
        {
            ...
            // Buttons
            {
                bool Button(Rect pos, string icon) =>
                    GUI.Button(pos, EditorGUIUtility.IconContent(icon));
                ...
                int i,
                    l;
                {
                    var transitions = _editor.GetStateTransitions(
        ...
        private static void SetupConditionsList(ReorderableList reorderableList)
        {
            ...
            ) =>
            {
                ...
                else
                {
                    EditorGUI.PropertyField(
                        new Rect(rect.x, rect.y, 150, rect.height),
                        condition,
                        GUIContent.none
                    );
                }
                ...
                EditorGUI.LabelField(
                    new Rect(rect.x + rect.width - 80, rect.y, 20, rect.height),
                    "Is"
                );
                EditorGUI.PropertyField(
                    new Rect(rect.x + rect.width - 60, rect.y, 60, rect.height),
                    prop.FindPropertyRelative("ExpectedResult"),
                    GUIContent.none
                );
                ...
                if (index < reorderableList.count - 1)
                    EditorGUI.PropertyField(
                        ...
                        prop.FindPropertyRelative("Operator"),
                        GUIContent.none
                    );
            };
            ...
            reorderableList.drawElementBackgroundCallback += (
                Rect rect,
                int index,
                bool isActive,
                bool isFocused
