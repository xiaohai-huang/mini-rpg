            ...
            // Add and cancel buttons
            {
                ...
                if (GUI.Button(position, "Add Transition"))
                {
                    if (SerializedTransition.FromState.objectReferenceValue == null)
                        ...
                        Debug.LogException(
                            new InvalidOperationException("FromState and ToState are the same.")
                        );
                    else
                    {
        ...
        private static void SetupConditionsList(ReorderableList reorderableList)
        {
            ...
            ) =>
            {
                ...
                if (condition.objectReferenceValue != null)
                {
                    ...
                    GUI.Label(rect, "If");
                    GUI.Label(
                        new Rect(rect.x + 20, rect.y, rect.width, rect.height),
                        label,
                ...
                EditorGUI.LabelField(
                    new Rect(rect.x + rect.width - 120, rect.y, 20, rect.height),
                    "Is"
                );
                EditorGUI.PropertyField(
