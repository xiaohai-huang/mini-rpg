...
namespace UOP1.StateMachine.Editor
{
    internal class TransitionTableEditorWindow : EditorWindow
    {
        ...
        private static readonly string _uxmlPath =
            "Assets/Scripts/StateMachine/Editor/TransitionTableEditorWindow.uxml";
        private static readonly string _ussPath =
            "Assets/Scripts/StateMachine/Editor/TransitionTableEditorWindow.uss";
        private bool _doRefresh;

        private UnityEditor.Editor _transitionTableEditor;

        [MenuItem("State Machine Editor", menuItem = "Mini RPG/State Machine Editor")]
        ...
        private void OnEnable()
        {
            ...
            rootVisualElement
                .Query<Label>()
                .Build()
                .ForEach(label => label.AddToClassList(labelClass));
        ...
        private void CreateListView()
        {
            ...
            if (_transitionTableEditor && _transitionTableEditor.target)
                listView.selectedIndex = System.Array.IndexOf(
                    assets,
                    _transitionTableEditor.target
                );
        }
        ...
        private void OnListSelectionChanged(IEnumerable<object> enumerable)
        {
            ...
            if (_transitionTableEditor == null)
                _transitionTableEditor = UnityEditor.Editor.CreateEditor(
                    table,
                    typeof(TransitionTableEditor)
                );
            else if (_transitionTableEditor.target != table)
                UnityEditor.Editor.CreateCachedEditor(
                    table,
                    typeof(TransitionTableEditor),
                    ref _transitionTableEditor
                );
        ...
        private TransitionTableSO[] FindAssets()
        {
            ...
            for (int i = 0; i < guids.Length; i++)
                assets[i] = AssetDatabase.LoadAssetAtPath<TransitionTableSO>(
