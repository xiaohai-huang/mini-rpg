...
namespace UOP1.StateMachine.Editor
{
    ...
    internal class TransitionTableEditor : UnityEditor.Editor
    {
        ...
        private void StateEditorGUI()
        {
            ...
            if (
                GUILayout.Button(
                    EditorGUIUtility.IconContent("scrollleft"),
                    GUILayout.Width(35),
                    GUILayout.Height(20)
                )
            ...
            for (int i = 0; i < _fromStates.Count; i++)
            {
                ...
                var headerRect = BeginHorizontal();
                {
                    ...
                    // Toggle
                    {
                        ...
                        _toggledIndex = EditorGUI.BeginFoldoutHeaderGroup(
                            toggleRect,
                            _toggledIndex == i,
                            label,
                            ContentStyle.StateListStyle
                        )
                            ? i
                            : _toggledIndex == i
                                ? -1
                    ...
                    // State Header Buttons
                    {
                        ...
                        {
                            DisplayStateEditor(
                                transitions[0].SerializedTransition.FromState.objectReferenceValue
                            );
                            EarlyOut();
                            return;
                        }
        ...
        {
            int stateIndex = _fromStates.IndexOf(
                serializedTransition.FromState.objectReferenceValue
            );
            var stateTransitions = _transitionsByFromStates[stateIndex];
            int index = stateTransitions.FindIndex(t =>
                t.SerializedTransition.Index == serializedTransition.Index
            );

            (int currentIndex, int targetIndex) = up
                ? (
                    serializedTransition.Index,
        ...
        internal void RemoveTransition(SerializedTransition serializedTransition)
        {
            ...
            if (index == 0 && count > 1)
                _transitions.MoveArrayElement(
                    stateTransitions[1].SerializedTransition.Index,
                    deleteIndex++
                );
            ...
            ApplyModifications(
                $"Deleted transition from {fromStateName} "
        ...
        {
            return _transitionsByFromStates[_fromStates.IndexOf(state)]
                .Select(t => t.SerializedTransition)
                .ToList();
        }
        ...
        private void CopyConditions(SerializedProperty copyTo, SerializedProperty copyFrom)
        {
            for (int i = 0, j = copyTo.arraySize; i < copyFrom.arraySize; i++, j++)
            {
                ...
                cond.FindPropertyRelative("ExpectedResult").enumValueIndex = srcCond
                    .FindPropertyRelative("ExpectedResult")
                    .enumValueIndex;
                cond.FindPropertyRelative("Operator").enumValueIndex = srcCond
                    .FindPropertyRelative("Operator")
                    .enumValueIndex;
                cond.FindPropertyRelative("Condition").objectReferenceValue = srcCond
                    .FindPropertyRelative("Condition")
                    .objectReferenceValue;
            }
        }

        private bool TryGetExistingTransition(
        ...
        )
        {
            ...
            toIndex = _transitionsByFromStates[fromIndex]
                .FindIndex(transitionHelper =>
        ...
        private void GroupByFromState()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    Debug.LogError(
                        "Transition with invalid \"From State\" found in table "
                ...
                {
                    Debug.LogError(
                        "Transition with invalid \"Target State\" found in table "
                            + serializedObject.targetObject.name
                            + ", deleting..."
                    );
                    _transitions.DeleteArrayElementAtIndex(i);
                ...
                if (
                    !groupedTransitions.TryGetValue(
                        serializedTransition.FromState.objectReferenceValue,
