namespace UOP1.StateMachine
{
    public class StateTransition : IStateComponent
    {
        ...
        )
        {
            ...
            _resultGroups =
                resultGroups != null && resultGroups.Length > 0 ? resultGroups : new int[1];
        ...
        {
#if UNITY_EDITOR
            _targetState._stateMachine._debugger.TransitionEvaluationBegin(
                _targetState._originSO.name
            );
#endif
            ...
            for (int j = 0; j < _resultGroups[i]; j++, idx++)
                _results[i] =
                    j == 0 ? _conditions[idx].IsMet() : _results[i] && _conditions[idx].IsMet();
...
#if UNITY_EDITOR
            _targetState._stateMachine._debugger.TransitionEvaluationEnd(
                ret,
                _targetState._actions
            );
#endif
