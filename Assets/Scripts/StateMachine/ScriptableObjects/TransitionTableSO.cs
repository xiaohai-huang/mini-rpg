...
namespace UOP1.StateMachine.ScriptableObjects
{
    ...
    {
        [SerializeField]
        private TransitionItem[] _transitions = default;
            ...
            foreach (var fromState in fromStates)
            {
                if (fromState.Key == null)
                    throw new ArgumentNullException(
                        nameof(fromState.Key),
                        $"TransitionTable: {name}"
                    );
                ...
                foreach (var transitionItem in fromState)
                {
                    ...
                    ProcessConditionUsages(
                        stateMachine,
                        transitionItem.Conditions,
                        createdInstances,
                        out var conditions,
                        out var resultGroups
                    );
        ...
        )
        {
            ...
            for (int i = 0; i < count; i++)
                conditions[i] = conditionUsages[i]
