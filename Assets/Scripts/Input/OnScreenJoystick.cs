...
#endif
////TODO: custom icon for OnScreenJoystick component
...
namespace Xiaohai.Input
{
    ...
    /// </summary>
    /// <remarks>
    /// The <see cref="OnScreenJoystick"/> works by simulating events from the device specified in the <see cref="OnScreenControl.controlPath"/>
    ...
    /// inputs from the previously active device are cancelled. In the case of <see cref="OnScreenJoystick"/>, this can mean that the
...
#if UNITY_EDITOR
        [CustomEditor(typeof(OnScreenJoystick))]
        internal class OnScreenJoystickEditor : UnityEditor.Editor
        {
            ...
            public override void OnInspectorGUI()
            {
                ...
                if (EditorGUILayout.BeginFadeGroup(m_ShowDynamicOriginOptions.faded))
                {
                    ...
                    {
                        ((OnScreenJoystick)target).UpdateDynamicOriginClickableArea();
                    }
                    EditorGUI.indentLevel--;
                }
